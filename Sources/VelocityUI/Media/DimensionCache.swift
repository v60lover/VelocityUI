...
public final class DimensionCache: Sendable {
    ...
    public func dimensions(for url: URL) async -> CGSize? {
        // Fast pre-lock read — avoids lock acquisition on the warm-cache hot path.
        // Working-range diff can re-request dimensions for the same URL on every layout pass.
