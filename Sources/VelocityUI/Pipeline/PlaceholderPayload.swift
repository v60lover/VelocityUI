...
/// is the open door for a consumer's own first-paint strategy, interpreted only by a
/// renderer injected through `RenderEnvironment`.
...
/// which must stay generic-free. Equality/hashing bridge through `AnyHashable`, which
/// already knows how to compare two `any Hashable` values of different concrete types.
