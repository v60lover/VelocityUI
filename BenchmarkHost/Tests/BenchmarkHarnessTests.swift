...
final class BenchmarkHarnessTests: XCTestCase {
    ...
    @MainActor
    func testRuntimeLabelAppearsInReport() {
        let harness = BenchmarkHarness()
        harness.runtimeLabel = "velocityui"
        harness.startCapture()
        let report = harness.stopCapture()
        XCTAssertEqual(report.runtime, "velocityui",
            "report.runtime must equal the label set before capture — not the default 'unknown'")
    }

    @MainActor
    func testDefaultRuntimeLabelIsUnknown() {
        let harness = BenchmarkHarness()
        harness.startCapture()
        let report = harness.stopCapture()
        XCTAssertEqual(report.runtime, "unknown",
            "default label must be 'unknown' when no runtime is set")
    }
    ...
    // MARK: - Thumbnail-to-image transition counter (VelocityUI-1su.3)

    @MainActor
    func testThumbnailTransitionCounterResetsOnStart() {
        let harness = BenchmarkHarness()
        harness.startCapture()
        let report = harness.stopCapture()
        XCTAssertEqual(report.thumbnailToImageTransitionCount, 0, "Fresh capture must start with zero count")
    }

    @MainActor
    func testThumbnailTransitionCounterIncrements() {
        let harness = BenchmarkHarness()
        harness.startCapture()
        let n = 4
        for _ in 0..<n { harness.recordThumbnailToImageTransition() }
        let report = harness.stopCapture()
        XCTAssertEqual(report.thumbnailToImageTransitionCount, n)
    }

    @MainActor
    func testThumbnailTransitionCounterIndependentOfGrayCounter() {
        // The two counters must track distinct dimensions — a thumbnail-replacement delivery
        // must not also count as a gray-replacement delivery, and vice versa.
        let harness = BenchmarkHarness()
        harness.startCapture()
        ...
        let report = harness.stopCapture()
        XCTAssertEqual(report.grayToImageTransitionCount, 2)
...
// MARK: - Discard-window tests

final class BenchmarkDiscardWindowTests: XCTestCase {

    private func makeTimestamps(count: Int, hz: Double = 60) -> [(ts: CFTimeInterval, target: CFTimeInterval)] {
        (0..<count).map { i in
            let t = Double(i) / hz
    ...
    func testDiscardWindowExcludesEarlyFrames() {
        // 120 frames @ 60 Hz = 2 s. Discard first 1 s:
        // timestamps[60].ts = 1.0 → startIdx = 60, effective = [60..119] = 60 entries → 59 intervals.
        let ts = makeTimestamps(count: 120)
        let stats = benchmarkComputeFrameStats(timestamps: ts, hitchSlack: 0.004, discardFirstSeconds: 1.0)
        XCTAssertEqual(stats.totalFrames, 59,
            "discard-first-second must leave 59 frame intervals from the second half")
    }

    func testDiscardWindowZeroPreservesAllFrames() {
        let ts = makeTimestamps(count: 60)
        let withZero = benchmarkComputeFrameStats(timestamps: ts, hitchSlack: 0.004, discardFirstSeconds: 0)
        let withDefault = benchmarkComputeFrameStats(timestamps: ts, hitchSlack: 0.004)
        XCTAssertEqual(withZero.totalFrames, 59)
        XCTAssertEqual(withDefault.totalFrames, withZero.totalFrames)
    }

    func testDiscardWindowLargerThanCaptureDurationReturnsEmpty() {
        let ts = makeTimestamps(count: 60)  // 1 s at 60 Hz
        let stats = benchmarkComputeFrameStats(timestamps: ts, hitchSlack: 0.004, discardFirstSeconds: 2.0)
        XCTAssertEqual(stats.totalFrames, 0)
        XCTAssertEqual(stats.hitchCount, 0)
    }

    @MainActor
    func testWarmupDiscardedSecondsPopulatedInReport() throws {
        let harness = BenchmarkHarness()
        harness.startCapture(discardFirstSeconds: 1.5)
        let report = harness.stopCapture()
        let v = try XCTUnwrap(report.warmupDiscardedSeconds,
            "warmupDiscardedSeconds must be non-nil when discardFirstSeconds > 0")
        XCTAssertEqual(v, 1.5, accuracy: 0.001)
    }

    @MainActor
    func testNilWarmupDiscardedSecondsWhenNoDiscard() {
        let harness = BenchmarkHarness()
        harness.startCapture()
        let report = harness.stopCapture()
        XCTAssertNil(report.warmupDiscardedSeconds,
            "no-discard capture must have nil warmupDiscardedSeconds")
    }
}
