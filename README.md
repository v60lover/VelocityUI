...
Four layers. Each one talks to the next through `Sendable` value types only — the Swift 6
compiler enforces that no mutable shared state crosses a boundary.

```
┌──────────────────────────────────────────────────────────────┐
│ Layer 1 · Developer DSL        @MainActor, SwiftUI-like       │
│   AsyncFeed + @RenderNodeBuilder                             │
│   VStack/HStack/ZStack · Text · AsyncImage/GIF/Video · Code  │
│   StreamingMarkdownText · HostingNode (UIView escape hatch)  │
└───────────────────────────────┬──────────────────────────────┘
                                │ NodeTable  (flat Sendable, no existentials)
┌───────────────────────────────▼──────────────────────────────┐
│ Layer 2 · Render Pipeline       off-main, parallel            │
│   Allocation-free differ + 3-tier change classifier          │
│   measureNode(): nonisolated pure fn, fanned out via TaskGroup│
│   TextMeasurementPool · LayoutCache (actor) · WorkingRange   │
└───────────────────────────────┬──────────────────────────────┘
                                │ ResolvedLayout  (O(1) ring-buffer lookup)
┌───────────────────────────────▼──────────────────────────────┐
│ Layer 3 · Scroll Container      @MainActor, UIScrollView      │
│   Custom cell lifecycle — NO UICollectionView                │
│   Pooled CALayer-backed RenderCells                          │
│   Scroll path: zero async hops, zero Task spawns             │
│   Two-phase commit: geometry first, media second            │
└───────────────────────────────┬──────────────────────────────┘
                                │ triggers fetch / decode
┌───────────────────────────────▼──────────────────────────────┐
│ Layer 4 · Media Pipeline        custom-executor actors        │
│   Network fetch (plain async) separated from decode          │
│   Decode on bounded DispatchQueues, off the cooperative pool │
│   ImageActor · GIFActor · VideoController + VideoPreparation │
│   Corner-rounding + BGRA8888 normalisation at decode time    │
└──────────────────────────────────────────────────────────────┘
```

### Composition root — no singletons

Every long-lived collaborator (caches, actors, pools, controllers) is owned by a single
`RenderEnvironment`, constructed **once per `AsyncFeed`** and injected by initializer. There is
no `static let shared` anywhere in VelocityUI-owned code. This is what lets two feeds run with
different memory budgets, lets a test swap in a fake `ImageActor`, and lets a feed tear down
cleanly with no leaked decode tasks or orphaned `AVPlayer`s.

```swift
let env = RenderEnvironment()            // auto-wires every collaborator
