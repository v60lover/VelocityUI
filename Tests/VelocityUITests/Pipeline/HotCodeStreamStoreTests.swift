...
final class HotCodeStreamStoreTests: XCTestCase {
    ...
    func testAppend_PreservesFinalGlyphAgainstTightRelayout_AndReportsBitmapWidth() {
        let store = HotCodeStreamStore()
        let key = BlockKey(itemID: "msg", index: 0)
        ...
        let measuredSize = measure(descriptor, .greatestFiniteMagnitude)
        let scale: CGFloat = 2

        // This is the regression oracle: the old per-line path re-laid out at measuredSize.width,
        ...
        XCTAssertGreaterThan(
            rightmostInkPixel(in: expectedImage) - rightmostInkPixel(in: tightImage), Int(scale.rounded()),
        ...
        guard let image = result.content.chunks.last?.image else {
            return XCTFail("expected a sealed hot code tile")
        }

        XCTAssertEqual(image.width, expectedImage.width, "hot raster must use the unbounded line layout")
        XCTAssertEqual(
            result.content.sealedSize.width, CGFloat(image.width) / scale, accuracy: 0.01,
            "hot sealed width must match the actual tile bitmap width"
        )
        XCTAssertEqual(
            result.content.totalSize.width, CGFloat(image.width) / scale, accuracy: 0.01,
            "horizontal scroll content width must reach the complete hot tile"
        )
        XCTAssertGreaterThanOrEqual(
            rightmostInkPixel(in: image), rightmostInkPixel(in: expectedImage) - 1,
            "the hot path must preserve the final glyph's ink"
        )
        XCTAssertEqual(result.content.sealedSize.height, font.uiFont.lineHeight, accuracy: 1,
                       "a sealed code line must remain one non-wrapping fragment")
    }
